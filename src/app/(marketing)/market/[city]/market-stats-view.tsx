...
export function MarketStatsView({ city, stats, seoContent }: MarketStatsViewProps) {
  ...
  const statCards = [
    ...
    { label: "Sold (12 mo)", value: latest.soldCount.toLocaleString(), icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z", color: "bg-purple-50 text-purple-600" },
  ...
  return (
    <div>
      ...
      {/* Real Estate Market Analytics source attribution (MLS Grid License
          Agreement — public analytics display must identify the source MLS,
          MLS GRID, and the time period). */}
      <p className="mb-8 text-xs text-slate-400">
        Market analytics — data courtesy of Stellar MLS as distributed by MLS GRID
        as of {latest.period}. Active-inventory figures reflect current listings;
        sold figures cover the trailing twelve months. Data is deemed reliable but
        not guaranteed.
      </p>
      ...
      {seoContent && (
        <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6 mb-8">
          <SanitizedHtml className="prose prose-slate prose-sm sm:prose-base max-w-none" html={seoContent} />
        </div>
      )}
