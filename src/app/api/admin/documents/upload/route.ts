...
export async function POST(request: NextRequest) {
  ...
  if (getAdminUploadExtension(filename) === null) {
    return NextResponse.json(
      { error: "File type not allowed", allowed: ALLOWED_EXTENSIONS },
      { status: 400 },
    );
  }
  ...
  const maxBytes = adminUploadLimitFor(filename, DOCUMENT_UPLOAD_LIMITS);
  ...
  if (error || !data) {
    logApiError("admin/documents/upload", error);
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof AdminUploadStorageError) {
      logApiError("admin/documents/upload-finalize", error);
