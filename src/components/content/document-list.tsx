...
function getViewerUrl(apiUrl: string): string | null {
  const prefix = "/api/documents/";
  if (!apiUrl.startsWith(prefix) || !apiUrl.endsWith(".pdf")) return null;
  const docPath = apiUrl.slice(prefix.length);
  return `/dashboard/documents/viewer?doc=${encodeURIComponent(docPath)}`;
}
