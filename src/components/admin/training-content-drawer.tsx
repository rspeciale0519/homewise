            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              {item?.slug && (
                <div className="flex items-center gap-2 -mt-2">
                  <a
                    href={
                      item.audience === "agent_only"
                        ? `/dashboard/training/${item.slug}`
                        : `/learn/${item.category}/${item.slug}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-navy-700 border border-slate-200 rounded-md hover:bg-slate-50 transition-colors"
                  >
                    Preview as {item.audience === "agent_only" ? "agent" : "public"}
